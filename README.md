...
pipeline = RecursiveSemanticDecomposition(
    ...
    ctfidf_top_n           = 5,
    random_state           = 42,
    use_llm_labels         = True,           # set False for c-TF-IDF only
    anthropic_api_key      = "sk-...",       # required if use_llm_labels=True
)
...
```python
# Flat DataFrame with cluster paths and assignment types
df = pipeline.to_dataframe()
df.to_excel("clustering_results.xlsx", index=False)

# Decomposition diagnostics (cluster sizes, DBCV scores per level)
...
```
.
├── code_cluster.ipynb    # Full pipeline notebook
├── README.md
├── requirements.txt
└── data/                 # Place your JSON requirement files here
    └── example.json
```
...
If you use this pipeline in academic work, please cite accordingly.
